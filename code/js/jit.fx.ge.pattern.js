...
var slab_resolve = new JitterObject("jit.gl.slab", drawto);
slab_resolve.file = "jit.fx.ge.pattern.resolve.jxs";
slab_resolve.inputs = 2;
slab_resolve.type = "float32";
fxobs.push(slab_resolve);
...
node_del.blend_enable = 1;
node_del.depth_enable = 0;
fxobs.push(node_del);
...
mesh_del.blend_enable = 1;
mesh_del.depth_enable = 0;
mesh_del.texture = [node_vor.out_name, inTex.name];
...
function setline_width(v){ 
	...
	slab_resolve.param("line_width", line_width);
...
function setline_growth(v){ 
	...
	slab_resolve.param("line_growth", line_growth);
}
...
function setline_fade(v){ 
	line_fade = v;
	slab_resolve.param("line_fade", line_fade);
...
function drawfx(inname){
	...
	switch(draw_mode) {
	  case 0:
	  	slab_resolve.activeinput = 1;
	  	slab_resolve.jit_gl_texture(inTex.name);
	  	slab_resolve.activeinput = 0;
		slab_resolve.jit_gl_texture(node_vor.out_name);
		slab_resolve.draw();
		outlet(0, "jit_gl_texture", slab_resolve.out_name);	
	    break;
	  case 1:
		node_del.draw();
	  ...
	  case 2:
		...
		slab_combine.jit_gl_texture(slab_resolve.out_name);
		slab_combine.draw();
